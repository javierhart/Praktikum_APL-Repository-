...
void hapus();
bool login();
int hitung_role_hero(const string& role, int index = 0, int count = 0); // Fungsi rekursif
...
void tambah() {
    ...
    hero[heroCount].abilities[0] = ability;
    heroCount++;
...
void edit() {
    ...
    if (pil > 0 && pil <= heroCount) {
        ...
        if (nama_baru != hero[pil - 1].nama && nama_duplikat(nama_baru, role)) {
...
void lihat_hero() {
    ...
    cout << "--------------------------------------------------------------------\n";
    for (int i = 0; i < heroCount; ++i) {
        cout << setw(20) << left << hero[i].nama << setw(20) << hero[i].role;
        cout << setw(15) << left << "Damage: " << hero[i].abilities[0].damage;
        cout << setw(10) << left << "Kesulitan: " << hero[i].abilities[0].difficulty;
        cout << endl;
    }
    cout << "Tekan Enter untuk kembali ke menu utama.\n";
    cin.ignore(numeric_limits<streamsize>::max(), '\n'); 
